...
export async function getBeerCounts(): Promise<BeerCountsResponse> {
  try {
    ...
    if (users.length === 0) {
      ...
      await Promise.all(
        defaultUsers.map(user => 
          db.collection("users").insertOne({ name: user })
        )
      )
    ...
    let peopleCounter = await db.collection("counters").find({}).toArray()
    ...
    if (!lastDateRecord || lastDateRecord.date !== today) {
      ...
      await db.collection("lastDate").updateOne(
        { id: "lastDate" },
        { $set: { date: today } },
        { upsert: true }
      )
...
export async function incrementBeer(person: Person): Promise<BeerCountsResponse> {
  try {
    ...
    await db.collection("dailyCounters").updateOne(
      { name: person, date: today }, 
      { $inc: { count: 1 } }, 
      { upsert: true }
    )

    // Update last action
    await db.collection("lastAction").updateOne(
      { id: "last" }, 
      { $set: { person, canUndo: true, daily: true } }, 
      { upsert: true }
    )
...
export async function undoLastAction(): Promise<BeerCountsResponse> {
  try {
    ...
    if (lastAction.daily) {
      await db.collection("dailyCounters").updateOne(
        { name: lastAction.person, date: today }, 
        { $inc: { count: -1 } }
      )
    }
