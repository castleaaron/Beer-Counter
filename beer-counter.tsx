...
import { Beer, Undo2, RefreshCw, UserPlus, UserMinus, RotateCcw, ShieldCheck, CalendarClock } from "lucide-react"
...
export default function BeerCounter() {
    ...
    if (emojiTapCount + 1 === 5) {
      setIsAdminMode(prev => {
        ...
        toast({
          ...
          description: newMode ? "Administrative functions are now available." : "Administrative functions are now hidden.",
        });
        return newMode;
      });
      setEmojiTapCount(0);
  ...
  const handleBeerIncrement = async (person: string) => {
    ...
    setAnimations(prev => ({ ...prev, [person]: true }))

    try {
      const result = await incrementBeer(person)
      ...
      toast({
        ...
        description: `Could not update ${person}'s beer count.`,
        variant: "destructive",
      })
    } finally {
      setIsUpdating(false)
      ...
      setTimeout(() => {
        setAnimations(prev => ({ ...prev, [person]: false }))
      }, 1000)
    }
  }
    ...
    <><div className="container mx-auto p-4 max-w-md bg-gradient-to-b from-amber-50 to-amber-100 min-h-screen rounded-xl shadow-inner">
      <div className="flex justify-between items-center mb-6 mt-4">
        ...
        <div className="flex gap-2">
          {isAdminMode && (
            <ShieldCheck className="h-5 w-5 text-amber-700 animate-pulse" />
          )}
          <Button
            variant="ghost"
            size="icon"
        ...
        <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.2 }}>
          <Card className="border-2 border-amber-300 bg-white/80 rounded-xl shadow-lg overflow-hidden">
            ...
            <CardContent className="p-4">
              ...
              <div className="text-center text-amber-600 mt-1 text-xs font-medium">
                {formatDate(today)}
              </div>
            </CardContent>
          </Card>
      ...
      <Tabs defaultValue="daily" className="mb-6">
        <TabsList className="w-full mb-4 bg-amber-100">
          <TabsTrigger value="daily" className="flex-1 data-[state=active]:bg-amber-500 data-[state=active]:text-white">
            Today's Leaders
          </TabsTrigger>
          <TabsTrigger value="alltime" className="flex-1 data-[state=active]:bg-amber-600 data-[state=active]:text-white">
            All-Time Leaders
          </TabsTrigger>
        </TabsList>
                  ...
                  {dailyTotalCount > 0 && (
                    ...
                    >
                      {dailyLeaders.length === 0 ? (
                        ...
                        <span className="capitalize">{dailyLeaders[0]} is winning today! 🏆 ({dailyMaxCount})</span>
                      ) : (
                        <span>
                          {dailyLeaders.map(name => (
                  ...
                  {totalCount > 0 && (
                    ...
                    >
                      {allTimeLeaders.length === 0 ? (
                        ...
                        <span className="capitalize">{allTimeLeaders[0]} is the all-time champion! 🏆 ({allTimeMaxCount})</span>
                      ) : (
                        <span>
                          {allTimeLeaders.map(name => (
                            <span key={name} className="capitalize">{name}</span>
                          )).reduce((prev, curr, i) => i === 0
                            ? curr
